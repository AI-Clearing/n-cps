...
with Engine(custom_parser=parser) as engine:
    ...
    train_loader, train_sampler = get_train_loader(engine, VOC, train_source=config.train_source, unsupervised=False)
    unsupervised_train_loader, unsupervised_train_sampler = get_train_loader(engine, VOC, train_source=config.unsup_source, unsupervised=True)
    ...
    for epoch in range(engine.state.epoch, config.nepochs):
        ...
        for idx in pbar:
            ...
            L_cps = calc_cps_loss(engine, criterion, pred_sup_list, pred_unsup_list)
